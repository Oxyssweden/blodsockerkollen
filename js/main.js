(function($) {
  ...
  var clock,
      EVENT_LOCATION = 0, EVENT_DATE = 1, EVENT_START = 2, EVENT_END = 3, EVENT_TEXT = 4,
   defaultText = 'Information om diabetes och gratis blodsockerkoll av sjuksköterska på plats. OBS! Det är dock viktigt att inte äta eller dricka något på en timme före mätning. Vatten går dock bra att dricka',
      timeIcon = '          <svg class="icon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 1000 1000" enable-background="new 0 0 1000 1000" xml:space="preserve">\n' +
          ...
          '          </svg>',
      mapIcon= '          <svg class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 1000 1000" enable-background="new 0 0 1000 1000" xml:space="preserve">\n' +
  ...
  req.onload  = function() {
    ...
    if (jsonResponse[0]) { setNextEvent(jsonResponse.shift()) }
    if (jsonResponse[0]) { setSchedule(jsonResponse) }
  }
  req.send(null);

  function setNextEvent(event) {
    eventNode = makeEvent(event);
  ...
  function makeEvent(event) {
    ...
    wrapper.append($('<div class="event__time">').html(timeIcon + event[EVENT_DATE]));

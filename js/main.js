(function($) {
  ...
  function calculateDates(event) {
    event[EVENT_START_TIMESTAMP] = moment.tz(event[EVENT_DATE] + ' ' + event[EVENT_START], "Europe/Stockholm").unix();
    event[EVENT_END_TIMESTAMP] = moment.tz(event[EVENT_DATE] + ' ' + event[EVENT_END], "Europe/Stockholm").unix();
    return event;
  }
  ...
  function makeEventNode(event) {
    ...
    dateString = event[EVENT_START].replace(':00', '') + '–' + event[EVENT_END] + ', '
